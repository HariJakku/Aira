    ...
    <div className="min-h-screen bg-white">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 glass safe-top">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <AiraLogo variant="symbol" size={36} />
            <AiraLogo variant="wordmark" size={34} />
          </div>
          ...
          <Link
            href="/customer/login"
            className="flex items-center gap-2 px-4 sm:px-5 py-2 sm:py-2.5 rounded-full text-sm font-medium text-white transition-all duration-200 hover:scale-105 hover:shadow-lg"
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-16">
        <div
          ...
          style={{
            backgroundImage: `
              radial-gradient(circle at 20% 50%, rgba(139,115,85,0.08) 0%, transparent 60%),
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 text-center">
          ...
          {/* Hero logo — full mark (symbol + Aira script) */}
          <div className="flex justify-center mb-6 sm:mb-8">
            <AiraLogo variant="mark" size={120} className="drop-shadow-sm" />
          </div>
          ...
          <h1
            ...
            style={{ fontSize: "clamp(2.4rem, 8vw, 5.5rem)", color: "var(--aira-dark)" }}
          >
            Curate Your{" "}
            <span className="text-gradient italic">Joy</span>
          </h1>

          {/* Subheading */}
          <p
            className="max-w-2xl mx-auto mb-8 sm:mb-10 leading-relaxed"
            style={{ fontSize: "clamp(0.95rem, 2vw, 1.15rem)", color: "var(--aira-text)", opacity: 0.7 }}
          >
          ...
          <div className="mt-14 sm:mt-20 grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
            {stats.map((s, i) => (
              ...
              >
                <div className="font-display text-2xl sm:text-3xl font-semibold" style={{ color: "var(--aira-gold)" }}>
                  {s.value}
                </div>
                <div className="text-xs sm:text-sm mt-1" style={{ color: "var(--aira-text)", opacity: 0.6 }}>
                  {s.label}
                </div>
              </div>
            ))}
      ...
      {/* ── ABOUT ───────────────────────────────────────────── */}
      <section id="about" className="py-20 sm:py-28 px-4 sm:px-6" style={{ background: "var(--aira-gold-pale)" }}>
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-2 gap-12 sm:gap-16 items-center">

            <div className="animate-on-scroll">
              <div className="text-xs font-semibold tracking-[0.3em] uppercase mb-4" style={{ color: "var(--aira-gold)" }}>
                About AIRA
              </div>
              <h2
                className="font-display leading-[1.15] mb-6"
                style={{ fontSize: "clamp(2rem, 4vw, 3.5rem)", color: "var(--aira-dark)" }}
              >
              ...
              <div className="flex flex-wrap gap-2 sm:gap-3">
                {["Birthdays", "Weddings", "Housewarmings", "Corporate Events"].map((tag) => (
                  <span
                    key={tag}
                    className="px-3 sm:px-4 py-1.5 rounded-full text-sm font-medium border"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              {features.map((f, i) => (
                <div
                  key={i}
                  className="card-premium p-4 sm:p-5 animate-on-scroll"
      ...
      <section id="how-it-works" className="py-20 sm:py-28 px-4 sm:px-6 bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12 sm:mb-16 animate-on-scroll">
            ...
            <h2 className="font-display leading-tight" style={{ fontSize: "clamp(2rem, 4vw, 3rem)", color: "var(--aira-dark)" }}>
              How AIRA Works
            </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {steps.map((step, i) => (
              <div
                key={i}
                className="card-premium p-6 sm:p-7 h-full animate-on-scroll"
                style={{ transitionDelay: `${i * 120}ms` }}
              >
                <div className="text-xs font-bold tracking-widest mb-4" style={{ color: "var(--aira-gold-muted)" }}>
                  {step.number}
                </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {portals.map((portal, i) => (
              ...
              >
                <div
                  className={`relative h-full rounded-2xl bg-gradient-to-br ${portal.color} border p-6 sm:p-7 transition-all duration-300 group-hover:shadow-xl group-hover:-translate-y-1`}
                  style={{ borderColor: "rgba(139,115,85,0.12)" }}
                >
                  <span
                    className="absolute top-4 right-4 text-xs font-semibold px-2 py-0.5 rounded-full"
                    style={{ background: "rgba(255,255,255,0.8)", color: portal.accent }}
                  >
                    {portal.badge}
                  </span>
                  <div className="w-12 h-12 rounded-2xl flex items-center justify-center mb-4 sm:mb-5" style={{ background: "rgba(255,255,255,0.85)" }}>
                  ...
                  <p className="text-sm leading-relaxed mb-4 sm:mb-5" style={{ color: "var(--aira-text)", opacity: 0.65 }}>
                    {portal.description}
                  </p>
      ...
      {/* ── FOOTER ──────────────────────────────────────────── */}
      <footer className="py-12 sm:py-16 px-4 sm:px-6" style={{ background: "var(--aira-dark)", color: "rgba(255,255,255,0.7)" }}>
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 sm:gap-12 mb-10 sm:mb-12">
            ...
            <div>
              <div className="flex items-center gap-3 mb-3">
                <AiraLogo variant="mark" size={48} className="opacity-90" />
                <AiraLogo variant="wordmark" size={40} light={true} showTagline={true} />
              </div>
            ...
            <div>
              ...
              <ul className="space-y-2">
                {portals.map((p) => (
                  <li key={p.id}>
                    <Link href={p.href} className="text-sm hover:text-white transition-colors duration-200 link-underline">
                      {p.label}
                    </Link>
                  </li>
                ))}
          ...
          {/* Bottom bar */}
          <div
            className="pt-6 sm:pt-8 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4 text-xs opacity-40"
