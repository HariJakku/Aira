...
import {
  ...
  Info, ZoomIn, Tag, Package, Ruler, Weight, Box,
} from "lucide-react";
...
/* ─── Modal ─────────────────────────────────────────────── */
function ImageModal({ item, onClose }: { item: GalleryImage; onClose: () => void }) {
  useEffect(() => {
  ...
  return (
    <div className="g-modal-bd" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="g-modal">
        <button className="g-modal-x" onClick={onClose}><X size={20} /></button>
        <div className="g-modal-grid">
          {/* Left: image */}
          <div className="g-modal-img-wrap">
            <Image src={item.image} alt={item.title} fill sizes="(max-width:640px) 100vw,50vw"
              className="object-contain" priority unoptimized={process.env.NODE_ENV === 'development'} />
          </div>
          {/* Right: info */}
          <div className="g-modal-info">
            ...
            {item.hasText && (
              <>
                <p className="g-ocr-lbl">Extracted Product Info (OCR)</p>
                <pre className="g-ocr-body">{item.text}</pre>
              </>
            )}
            <Link href="/customer/login" className="g-modal-cta">Request This Item →</Link>
          </div>
        </div>
      </div>
    </div>
...
function GalleryCard({
  item, onClick, isHovered, anyHovered, onEnter, onLeave,
}: {
  item: GalleryImage; onClick: () => void;
  ...
  return (
    <div
      ref={ref}
      className={`g-card ${inView ? "g-card--in" : ""} ${anyHovered && !isHovered ? "g-card--dim" : ""} ${isHovered ? "g-card--hov" : ""}`}
    ...
    >
      ...
      {/* Gold info dot if item has OCR text */}
      {item.hasText && <div className="g-card-dot"><Info size={9} /></div>}
    </div>
  );
}

/* ─── Row ───────────────────────────────────────────────── */
function GalleryRow({ section, items, onCardClick }: {
  section: string; items: GalleryImage[]; onCardClick: (i: GalleryImage) => void;
}) {
  const [hov, setHov] = useState<number | null>(null);
  const strip = useRef<HTMLDivElement>(null);
  const sc = (d: "l" | "r") => strip.current?.scrollBy({ left: d === "r" ? 640 : -640, behavior: "smooth" });

  return (
    <div className="g-row group/row">
      <h2 className="g-row-h">{section}</h2>
      <div className="g-strip-wrap">
        <button className="g-arr g-arr--l" onClick={() => sc("l")}><ChevronLeft size={18} color="white" /></button>
        <button className="g-arr g-arr--r" onClick={() => sc("r")}><ChevronRight size={18} color="white" /></button>
        <div ref={strip} className="g-strip">
          {items.map((item, i) => (
            <GalleryCard key={item.id} item={item} onClick={() => onCardClick(item)}
              isHovered={hov === i} anyHovered={hov !== null}
              onEnter={() => setHov(i)} onLeave={() => setHov(null)} />
          ))}
        </div>
      </div>
    </div>
...
export default function GalleryPage() {
  ...
  const fetch_ = useCallback(async (cat: string, query: string) => {
    setLoading(true);
    try {
    ...
    } catch { setError("Could not load gallery. Make sure the dev server is running."); }
    finally { setLoading(false); }
  }, []);

  useEffect(() => { fetch_("all", ""); }, [fetch_]);

  useEffect(() => {
    const t = setTimeout(() => fetch_(activeCat, q), 320);
  ...
  return (
    <>
      <style>{`
        ...
        /* Skeleton */
        .g-skel { animation:skelPls 1.6s ease infinite;
        ...
        /* When another card is hovered, dim this one — but keep image visible */
        ...
        /* Image wrapper — always fills the card */
        .g-card-img-wrap {
        ...
        /* Always-visible gradient + title at bottom */
        .g-card-label {
        ...
        .g-card--hov .g-card-label { opacity:0; }  /* hide when overlay shows */
        .g-card-label-text {
        ...
        .g-modal-grid { display:grid; grid-template-columns:1fr 1fr; min-height:500px; }
        ...
        .g-modal-cat {
          ...
          padding:3px 10px; border:1px solid rgba(139,115,85,.4); border-radius:20px; width:fit-content;
        }
      ...
      <div className="min-h-screen" style={{ background: "var(--aira-dark)" }}>
        ...
        <div
          ...
          style={{ background: "rgba(0,0,0,0.6)", backdropFilter: "blur(16px)", borderBottom: "1px solid rgba(255,255,255,0.07)" }}
        >
          ...
          <Link href="/customer/login" className="text-xs font-semibold px-4 py-2 rounded-full"
        ...
        {/* ── HERO ── */}
        <div className="relative flex items-end px-8 pb-10 overflow-hidden" style={{ height: "32vh", minHeight: "200px" }}>
          ...
          <div className="relative z-10 w-full flex flex-col sm:flex-row sm:items-end sm:justify-between gap-5">
            <div>
              ...
              <p className="mt-2 text-sm" style={{ color: "var(--aira-text)", opacity: 0.58 }}>
                {loading ? "Loading products…" : `${images.length} products · hover to preview specs · click for full details`}
              </p>
            </div>
            <div className="g-srch-wrap">
              <Search size={15} className="g-srch-ico" />
              <input className="g-srch" placeholder="Search products, materials…" value={q} onChange={(e) => setQ(e.target.value)} />
              {q && <button className="g-srch-clr" onClick={() => setQ("")}><X size={14} /></button>}
            </div>
          </div>
        </div>
        ...
        {!loading && (
          <div className="g-chips">
            <button className={`g-chip ${activeCat === "all" ? "g-chip--on" : ""}`} onClick={() => setActiveCat("all")}>
              All ({images.length})
            </button>
            {categories.map((cat) => (
              <button key={cat} className={`g-chip ${activeCat === cat ? "g-chip--on" : ""}`}
        ...
        {error ? (
          <div style={{ textAlign: "center", padding: "80px 20px", color: "rgba(255,255,255,.5)" }}>
            ...
            <p>{error}</p>
            <button onClick={() => fetch_(activeCat, q)}
          ...
          <div className="py-2">
            {rows.map((row, i) => (
              <GalleryRow key={row.section + i} section={row.section} items={row.items} onCardClick={setModal} />
            ))}
          </div>
        )}
      ...
      {modal && <ImageModal item={modal} onClose={() => setModal(null)} />}
    </>
  );
}
