...
function GalleryInner() {
  ...
  // Deep-link: /gallery?product=I3 opens that product directly
  useEffect(() => {
    ...
    <>
      <style>{`
        ...
        .g-card {
          ...
          transition:transform .38s cubic-bezier(.22,1,.36,1),opacity .38s ease,box-shadow .35s ease;
        }
        ...
        .g-card-label-text { color:#fff; font-size:11.5px; font-weight:600; line-height:1.35; text-shadow:0 1px 4px rgba(0,0,0,.7); }
        ...
        .g-chip-count { display:inline-block; margin-left:5px; font-size:10px; opacity:.65; font-weight:500; }
        ...
        .g-modal-img-el { object-fit:contain; transition:transform .4s cubic-bezier(.22,1,.36,1); }
        ...
        .g-modal-share {
          ...
          border-radius:20px; padding:4px 11px; cursor:pointer; transition:all .2s;
        }
      ...
      <div className="min-h-screen" style={{ background: "var(--aira-dark)" }}>
          ...
          <div className="relative z-10 w-full flex flex-col sm:flex-row sm:items-end sm:justify-between gap-5">
            <div>
              <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: ".32em", textTransform: "uppercase", color: "var(--aira-gold)", display: "block", marginBottom: 8 }}>
                ✦ Real Products · Handcrafted · Premium Gifting
              </span>
              ...
              <p className="mt-2 text-sm" style={{ color: "var(--aira-text)", opacity: 0.58 }}>
                {loading ? "Loading products…" : `${images.length} products · hover to preview · click for full details`}
              </p>
            </div>
        ...
        {!loading && (
          <div className="g-chips">
            <button className={`g-chip ${activeCat === "all" ? "g-chip--on" : ""}`} onClick={() => setActiveCat("all")}>
              All <span className="g-chip-count">({images.length})</span>
            </button>
            {categories.map((cat) => (
              <button
                key={cat}
              ...
              >
                {cat} <span className="g-chip-count">({catCounts[cat] ?? 0})</span>
              </button>
            ))}
          </div>
        )}
        ...
        {error ? (
          <div style={{ textAlign: "center", padding: "80px 20px", color: "rgba(255,255,255,.5)" }}>
            ...
            <p>{error}</p>
            <button onClick={() => fetchImages(activeCat, q)}
        ...
        <div className="text-center py-10 text-xs"
          ...
          © {new Date().getFullYear()} AIRA · Premium Handcrafted Gift Collection
        </div>
      </div>
